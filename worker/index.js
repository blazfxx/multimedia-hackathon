export default {
  async fetch(request, env) {
    ...
    if (path === "/groq/transcriptions" && request.method === "POST") {
      return proxyGroq(request, env);
    }
    if (path === "/nvidia/chat/completions" && request.method === "POST") {
      return proxyNvidia(request, env);
    }
    if (path === "/pexels/videos" && request.method === "GET") {
      return proxyPexels(request, url, env);
    }
...
async function proxyNvidia(request, env) {
  ...
  if (isStream) {
    ...
    return new Response(readable, {
      headers: { "Content-Type": "text/event-stream", ...corsHeaders(request) }
    });
  }
  const data = await res.text();
  return new Response(data, {
